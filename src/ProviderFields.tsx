...
export function ProviderFields({ value, onChange, readOnly = false }: { value: ProviderProfile; onChange: (value: ProviderProfile) => void; readOnly?: boolean }) {
  ...
  return <>
    ...
    <Field label="服务商协议" hint="网关根据调用端点自动转换请求、响应和流式事件；供应商认证仍使用 Cloudflare BYOK 的配置。"><Select disabled={readOnly} value={value.protocol} onChange={protocol => onChange({ ...value, protocol: protocol as ProviderProfile['protocol'] })}><option value="openai">OpenAI 兼容 · Chat Completions</option><option value="anthropic">Anthropic · Messages</option></Select></Field>
